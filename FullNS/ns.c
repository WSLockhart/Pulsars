  ...
  // if (distance<rho_spot)
    {
      ...
      */
      // double Tbbsur=Tbb*(1-(distance/rho_spot)); // simple test of non-uniform temperature (NB: 'fakedip')
      // intensity = Tbbsur;

      //NB: must pass phi+phi0 to account for the star's rotation!
      double JJ = current_density(angle_theta,angle_phi+phi0);
      double Tbbsur = pow(JJ,(1./8)); //j~T^4
      intensity=energy*energy*energy/(exp(energy/Tbbsur)-1.0);
      // double intensity = JJ;

      // intensity=energy*energy*energy/(exp(energy/Tbb)-1.0);  //constant-temp baseline to compare (NB: 'flatT')

      // comment out the following to make it isotropic
      // otherwise it becomes the beaming function of Thomson scattering
      //   intensity*=(1+2.145*beamangle-0.2*beamangle*beamangle)
      // /(2.*(1./2.+2.154/3.-0.2/4.));
