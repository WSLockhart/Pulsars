...
/*  NB: March 2019 (WL)
    This script is no longer used. Instead the inputs are passed directly
    to ray.c when the program is called.
*/

{
  ...
  if (in==NULL)                        // error handling
    {
      fprintf(stderr,"Error in opening input file <in>\n");
      return;
    }
  ...
  if (fclose(in)!=0)                  // close input file
    fprintf(stderr,"Error in closing input file <in>\n");
