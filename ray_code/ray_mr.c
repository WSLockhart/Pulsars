...
extern void time_spectrum(double Energy[], double Time[], double dyn_spectra[Nt][Nen]);
                                                  // from time.c
...
int main(void)
{
  double Energy[Nen];                   // grid of photon energies for spectru
  ...
  for (mns=1.0;mns<=3.2;mns+=0.2)
    {
      for (rcirc_ns=5.0;rcirc_ns<=15.0;rcirc_ns+=1.0)
	{

	  if (rcirc_ns>4.43*mns)    // if outside 3M
	  ...
	  Omegans=3.09646e-5*fns*mns;
	  
  //  Set the energy grid
  set_energy_grid(Energy);
  //  Set the time grid
  set_time_grid(Time);

  time_spectrum(Energy,Time,dyn_spectra);
